...
- [ ] Select build `3.16.1 (31601)` after processing completes
- [ ] Set app availability and pricing (Free) deliberately
- [ ] Add build 31601 to an internal TestFlight group
...
- [ ] Verify standard Quick reports save without a photo or location, including eligible issue selections when the test-email opt-in is off; verify the optional test toggle stays disabled until a relevant picture and confirmed location within exactly 5 km are present
- [ ] Confirm only explicitly opted-in qualifying payloads encode `official_email_destination_authorized: "test"`, all others encode null, and this 3.16.1 build never emits `official`
- [ ] Confirm every disclosure and post-save status says version 3.16.1 uses only a ColumbiaWalks-controlled test mailbox with a `[TEST]` subject—not Police, the Mayor, or Codes—and does not claim or guarantee email delivery
...
- [ ] Confirm the standalone Community police tool makes no ColumbiaWalks request; confirm Submit to CW & Notify CBPD saves only the CW report before preparing the separate draft; verify neither path sends a tip or media to CBPD, both require past/not-in-progress confirmation, and only verified official Police Department URLs open after a not-submitted warning
...
- [ ] Verify Contact Us displays Robert at (717) 466-9069, Call and Text open the expected system apps, and the private-message path opens App Feedback
- [ ] Complete large Dynamic Type, VoiceOver, landscape, simulator, and physical-iPhone QA for all four Community destinations
